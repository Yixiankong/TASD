#!/bin/bash
# =============================================================================
# GRPO Baseline 超参扫描 - Nebula 批量提交
#
# 参考 experiments/generalization/run_baseline_grpo_all_local.sh
# 使用方式：bash nebula_scripts/submit_grpo_baseline_sweep.sh [--dry-run]
# =============================================================================
...
# =============================================================================
# 超参配置（与 run_baseline_grpo_all_local.sh 保持一致）
# =============================================================================
...
for MINI_BATCH_SIZE in "${MINI_BATCH_SIZES[@]}"; do
    ...
    if [ "$DRY_RUN" = true ]; then
        ...
        echo "  DATASET=$DATASET MODEL=$MODEL_NAME LR=$LR MBS=$MINI_BATCH_SIZE"
    else
        ...
        SUBMIT_OUTPUT=$(nebulactl run mdl \
            ...
            --user_params="--script_path=${SCRIPT_PATH} --world_size=${WORLD_SIZE} --job_name=${JOB_NAME} --env=PROJECT_NAME=${PROJECT_NAME} --env=JOB_NAME=${JOB_NAME} --env=DATASET=${DATASET} --env=MODEL_NAME=${MODEL_NAME} --env=LR=${LR} --env=MINI_BATCH_SIZE=${MINI_BATCH_SIZE} --env=TRAIN_BATCH_SIZE=${TRAIN_BATCH_SIZE} --env=ROLLOUT_N=${ROLLOUT_N}" \
