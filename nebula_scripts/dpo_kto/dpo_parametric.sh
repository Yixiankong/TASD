#!/bin/bash
# DPO Nebula parametric training script
# Called by submit_dpo_sweep.sh
...
--run_name DPO-${MODEL_SHORT}-${THINK_MODE}-beta${BETA}-lr${LR}-bs${BATCH_SIZE}-${LOSS_TYPE} \
--bf16 \
...
if [ "$N_GPUS" -gt 1 ]; then
    ...
    accelerate launch \
        ...
        ${REPO_ROOT}/training/dpo_train.py $ARGS
else
    # Single GPU
    accelerate launch \
