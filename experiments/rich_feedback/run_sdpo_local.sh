...
ALPHAS=(1.0)           # 0: forward KL, 0.5: Jensen-Shannon, 1: reverse KL
LAMBDAS=(0.0)          # EXP_NAME 标识用
CLIP_ADV_HIGHS=(null)  # EXP_NAME 标识用
                        ...
                        for LAMBDA in "${LAMBDAS[@]}"; do
                            for CLIP_ADV_HIGH in "${CLIP_ADV_HIGHS[@]}"; do
                                for DATA_PATH in "${DATA_PATHS[@]}"; do
                                    ...
                                    EXP_NAME="FINAL-SDPO-${GPU_TYPE}-${DATASET_NAME}-train${TRAIN_BATCH_SIZE}-alpha${ALPHA}-rollout${ROLLOUT_BATCH_SIZE}-lr${LR}-lambda${LAMBDA}-clip${CLIP_ADV_HIGH}-dross${DONTS_REPROMPT_ON_SELF_SUCCESS}-${MODEL_NAME}-$(date +%Y-%m-%d_%H-%M-%S)"
