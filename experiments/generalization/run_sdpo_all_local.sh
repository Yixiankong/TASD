...
# True: 不对自身成功的样本进行 reprompt
DONTS_REPROMPT_ON_SELF_SUCCESSS=(True)

# 0: forward KL, 0.5: Jensen-Shannon, 1: reverse KL
ALPHAS=(0.5)
                ...
                for MODEL_PATH in "${MODEL_PATHS[@]}"; do
                    for ALPHA in "${ALPHAS[@]}"; do
                        for DATA_PATH in "${DATA_PATHS[@]}"; do
                            ...
                            EXP_NAME="FINAL-SDPO-${DATASET_NAME}-train${TRAIN_BATCH_SIZE}-alpha${ALPHA}-rollout${ROLLOUT_BATCH_SIZE}-lr${LR}-dross${DONTS_REPROMPT_ON_SELF_SUCCESS}-${MODEL_NAME}-$(date +%Y-%m-%d_%H-%M-%S)"
